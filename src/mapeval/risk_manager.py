...
class RiskManager:
    ...
    def check_drawdown(self, current_equity: float) -> tuple[bool, str | None]:
    ...
    def check_daily_loss(self, current_equity: float) -> tuple[bool, str | None]:
        ...
        if daily_loss > self.limits.max_daily_loss:
            return (
                False,
                f"Daily loss limit exceeded: {daily_loss:.2%} > {self.limits.max_daily_loss:.2%}",
            )
        return True, None

    def check_equity_floor(
        self, current_equity: float, initial_equity: float
    ) -> tuple[bool, str | None]:
        if initial_equity <= 0:
            return True, None
    ...
    def check_position_duration(
        ...
        if duration > self.limits.max_position_duration_seconds:
            return (
                False,
                f"{symbol} position held too long: {duration / 3600:.1f}h > {self.limits.max_position_duration_seconds / 3600:.1f}h",
            )
        return True, None
    ...
    def check_cooldown(self, current_time: datetime) -> tuple[bool, str | None]:
    ...
    def get_position_age_warnings(self, current_time: datetime) -> list[str]:
        warnings = []
        for symbol, _open_time in self.state.position_open_times.items():
