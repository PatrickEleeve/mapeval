...
class PerformanceAnalyzer:
    ...
    def calculate_sharpe_ratio(self, returns: list[float] | None = None) -> float | None:
        if returns is None:
    ...
    def calculate_sortino_ratio(self, returns: list[float] | None = None) -> float | None:
        if returns is None:
            returns = self.calculate_returns()
        if len(returns) < 2:
            return None
    ...
    def calculate_max_drawdown(self) -> tuple[float, pd.Timestamp | None, pd.Timestamp | None]:
    ...
    def calculate_calmar_ratio(self) -> float | None:
        ...
        annualized_return = (
            (1 + total_return) ** (1 / duration_years) - 1 if duration_years >= 1 else total_return
        )
    ...
    def calculate_win_rate(self) -> float | None:
    ...
    def calculate_profit_factor(self) -> float | None:
        if not self.trade_log:
            return None
    ...
    def analyze_decision_quality(self) -> dict:
        ...
        confidences = [
            d.get("confidence", 0) for d in self.decision_log if d.get("confidence") is not None
        ]
    ...
    def calculate_turnover(self) -> float | None:
