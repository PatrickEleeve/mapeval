...
class AdaptiveIntervalManager:
    ...
    def calculate_portfolio_volatility(self) -> float | None:
        volatilities = []

        for _symbol, history in self.price_history.items():
            if len(history) < 5:
                continue
    ...
    def get_recommended_interval(self) -> float:
        ...
        if volatility > self.volatility_threshold_high:
            ...
            self.last_adjustment_reason = (
                f"high volatility ({volatility:.4f}), using minimum interval"
            )
        elif volatility < self.volatility_threshold_low:
            new_interval = self.max_interval_seconds
            self.last_adjustment_reason = (
                f"low volatility ({volatility:.4f}), using maximum interval"
            )
        else:
            ...
            self.last_adjustment_reason = (
                f"moderate volatility ({volatility:.4f}), scaling interval"
            )

        new_interval = max(self.min_interval_seconds, min(self.max_interval_seconds, new_interval))

        smoothing = 0.3
        self.current_interval = smoothing * new_interval + (1 - smoothing) * self.current_interval

        return self.current_interval

    def detect_regime_change(self) -> str | None:
        volatility = self.calculate_portfolio_volatility()
        ...
        if volatility < self.volatility_threshold_low * 0.5:
