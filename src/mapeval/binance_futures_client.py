...
class BinanceFuturesClient:
    ...
    def cancel_all_orders(self, symbol: str) -> dict[str, Any]:
        ...
        return self._request(
            "DELETE", "/fapi/v1/allOpenOrders", params={"symbol": symbol.upper()}, weight=1
        )
    ...
    def get_open_orders(self, symbol: str | None = None) -> list[dict[str, Any]]:
        ...
        return self._request(
            "GET", "/fapi/v1/openOrders", params=params, weight=1 if symbol else 40
        )
    ...
    def get_exchange_info(self) -> dict[str, Any]:
        ...
        if self._exchange_info_cache is None:
            self._exchange_info_cache = self._request(
    ...
    def normalize_order(
        ...
        if rules["max_qty"] > 0 and normalized_qty > rules["max_qty"]:
            raise ValueError(
                f"{symbol} quantity {normalized_qty} exceeds maxQty {rules['max_qty']}"
            )
        if normalized_price is not None:
            if rules["min_price"] > 0 and normalized_price < rules["min_price"]:
                raise ValueError(
                    f"{symbol} price {normalized_price} below minPrice {rules['min_price']}"
                )
            if rules["max_price"] > 0 and normalized_price > rules["max_price"]:
                raise ValueError(
                    f"{symbol} price {normalized_price} exceeds maxPrice {rules['max_price']}"
                )
            if (
                rules["min_notional"] > 0
                and normalized_qty * normalized_price < rules["min_notional"]
            ):
                raise ValueError(
