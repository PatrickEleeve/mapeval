...
class PositionSizer:
    ...
    def create_pyramid_plan(
        ...
        for i in range(self.pyramid_levels):
            ...
            if i == 0:
                ...
                size_fraction = (remaining_fraction * self.scaling_factor) / (
                    self.pyramid_levels - i
                )
    ...
    def get_pyramid_status(self, symbol: str) -> dict | None:
        if symbol not in self.active_pyramids:
            return None
        ...
        return {
            ...
            "levels_triggered": sum(1 for level in plan.levels if level.triggered),
            "total_levels": len(plan.levels),
            "fill_percentage": (plan.total_filled / plan.base_size) * 100
            if plan.base_size > 0
            else 0,
        }
