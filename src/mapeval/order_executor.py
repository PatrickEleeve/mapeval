...
class GuardedOrderExecutor(OrderExecutor):
    """Executor wrapper that blocks state-changing operations when guarded."""

    def __init__(self, delegate: OrderExecutor, guard) -> None:
        self._delegate = delegate
        self._guard = guard

    def submit_order(self, order: Order) -> OrderResult:
        self._guard.check("submit_order")
        return self._delegate.submit_order(order)

    def cancel_order(self, symbol: str, order_id: str) -> bool:
        self._guard.check("cancel_order")
        return self._delegate.cancel_order(symbol, order_id)
    ...
    def sync_balance(self) -> float:
        return self._delegate.sync_balance()

    def get_execution_mode(self) -> str:
        return self._delegate.get_execution_mode()
...
class SimulatedExecutorBase(OrderExecutor):
    ...
    def _simulate_fill(self, order: Order, order_id_prefix: str) -> OrderResult:
        ...
        self._apply_fill_to_book(order, exec_price, commission)
...
class PaperExecutor(SimulatedExecutorBase):
    ...
    def _apply_fill_to_book(self, order: Order, exec_price: float, commission: float) -> None:
        ...
        direction = 1.0 if existing.quantity > 0 else -1.0
        realized = (exec_price - existing.entry_price) * closed_qty * direction
        self._balance += realized - commission
        remaining_qty = existing.quantity + signed_qty

        if abs(remaining_qty) < 1e-12:
            self._positions.pop(order.symbol, None)
            return

        if existing.quantity * remaining_qty > 0:
...
class LiveExecutor(OrderExecutor):
    ...
    def submit_order(self, order: Order) -> OrderResult:
        try:
            normalized = self._client.normalize_order(
                symbol=order.symbol,
            ...
            response = self._client.place_order(
                ...
                price=normalized["price"] if order.order_type == OrderType.LIMIT else None,
